...
class agent:
    ...
    def set_up(self):
        ...
        self.agent1.add(FC_Layer((24,1), bias_true=False))
    ...
    def get_action(self, state1, state2):
        ...
        print(f"move {sigmoid(prediction[0][0][0])}")
        self.move1 = bool(sigmoid(round(prediction[0][0][0])))
        
        #state2 = np.array(state1, dtype = float)
        #prediction = self.agent2.forward_prop(state2)
        #self.move2 = bool(round(prediction[0]))
        self.move2 = bool(sigmoid(round(prediction[0][0][0])))
