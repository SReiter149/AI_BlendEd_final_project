...
class Conv_Layer: 
    def __init__ (self, filt_shape, weights,bias): #filt_shape = [num_filts,num_channels, filt_x, filt_y]
        self.weights = weights
        self.bias = bias
    
    def forward_prop(self, input, test = False): #inputs = [num_images, num_layers, layer_x, layer_y]
        output = np.zeros((input.shape[0],self.weights.shape[0],input.shape[2] - self.weights.shape[1] + 1, input.shape[3] - self.weights.shape[2] + 1))
...
class Max_pooling:
    ...
    def forward_prop(self,input, test = False):
        output = np.zeros((input.shape[0],input.shape[1],int(math.ceil(input.shape[2]/2)), int(math.ceil(input.shape[3]/2))))     
        for i in range(0,input.shape[2],self.step_size):
            for j in range(0,input.shape[3],self.step_size):
                ...
                output[:,:,i//self.step_size,j//self.step_size] = np.amax(subsection,axis = (2,3))
        return output
